...
@pattern
def dm_evaluate(dm: OpenSourceAgent, attacker: str, batman_hp: int, wolfman_hp: int):
    """DM evaluates combat outcome - wrapped in pattern to track messages"""
    result = dm.run(
        f"Describe the outcome of {attacker}'s action. Current HP: Batman {batman_hp}, Wolfman {wolfman_hp}",
        cli="no-tools",
...
with session() as s:
    ...
    while batman_hp > 0 and wolfman_hp > 0:
        ...
        result = dm_evaluate(dm, "Batman", batman_hp, wolfman_hp)  # Now tracked!
        ...
        result = dm_evaluate(dm, "Wolfman", batman_hp, wolfman_hp)  # Now tracked!
